...
int main(int argc, char **argv) {
    {
        ...
        ASSERT(sizeof(vec) == 24, "Fill in the correct value."); // 指针、大小、容量
        int ans[]{1, 2, 3, 4, 5};
        ASSERT(std::memcmp(vec.data(), ans, vec.size() * sizeof(int)) == 0, "Fill in the correct values.");
    }
    {
        std::vector<double> vec{1, 2, 3, 4, 5};
        {
            ...
            ASSERT(std::memcmp(vec.data(), ans, vec.size() * sizeof(double)) == 0, "Fill in the correct values.");
        }
        {
            vec.push_back(6);
